	...
	void Serialize( const TCHAR* Data, enum EName Event )
	{
		...
		if( !GIsCriticalError || Entry )
		{
			//if( !FName::SafeSuppressed(Event) )
			{
				...
				{
					// Open log file.
					FString LogFile = FString(appBaseDir()) + TEXT("Output.log");
					LogAr = GFileManager->CreateFileWriter( *LogFile, FILEWRITE_AllowRead|FILEWRITE_Unbuffered|(Opened?FILEWRITE_Append:0));
					if( LogAr )
					{
						Opened = 1;
