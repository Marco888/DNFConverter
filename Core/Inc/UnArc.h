...
public:
	...
	friend FArchive& operator<<( FArchive& Ar, wchar_t& S )
	{
		Ar.ByteOrderSerialize( &S, sizeof(S) );
		return Ar;
	}
...
// Following taken from UnShock.
inline void IndexSer( FArchive& Ar, INT& Reference )
{
	INT	Index;
	BYTE	B;
	DWORD	Data, Shift;
	...
	// NOTE: 1. Byte: 8. Bit: Sign-Bit, 7. Bit: Next-Bit, 1. - 6. Bit: Index-Data
	// alle weiteren: 8. Bit: Next-Bit, 1. - 7. Bit: Index-Data

	Ar << B;

	Neg = B & ( 1<<7 );
	Index = B & 0x3f;	// 0x3f ist die Bitmaske vom 1. - 6. Bit
