...
inline INT appAtoi( const TCHAR* String ) { return _wtoi( String ); }
inline QWORD appAtol( const TCHAR* String ) { return _wtoi64( String ); }
...
inline TCHAR* appFromAnsi( ANSICHAR* C )
{
	static TCHAR Result[2048];

	INT i=0;
	for( i=0; i<2047 && C[i]; ++i )
		Result[i] = C[i];
	Result[i] = 0;
	return Result;
}
inline ANSICHAR* appToAnsi( TCHAR* C )
{
	static ANSICHAR Result[2048];
