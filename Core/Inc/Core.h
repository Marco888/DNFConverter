...
inline TCHAR* GetStrID( ENetID ID )
{
...
enum {MaxPacket = 512					};
enum{ MAX_CHANNELS         = 1023  };

class TcpIpConnectionBase
{
public:
	INT Port,PendingOffset;
	DWORD Addr;
	DOUBLE LastUpdateTime;
	FString PendingLine;
	bool bAccepted,bHasPinged,bDataPending;
	void* Data;

	TcpIpConnectionBase( DWORD Ad, INT P )
		: Port(P)
		, Addr(Ad)
		, bAccepted(false)
		, Data(NULL)
		, bDataPending(false)
	{}
	virtual FString IpToString()=0;
	virtual void SendBytes( BYTE* Data, INT Size )=0;
	virtual DWORD IpAddrToDWORD()=0;
	inline void SendChar( BYTE C )
	{
		SendBytes(&C,1);
	}
};
struct FNetworkHandleBase
{
	virtual void ReceiveBytes( TcpIpConnectionBase* Con, BYTE* Data, INT Size )=0;
};
class TcpIpNetBase
{
public:
	INT Port;
	FNetworkHandleBase* Handle;

	virtual bool ConnectionOpen( INT PortNum, FString& Error, const TCHAR* TargetAddr )=0;
	virtual void Tick( FLOAT Delta )=0;
	virtual TcpIpConnectionBase* GetConnection()=0;
};

TcpIpNetBase* InitConnection( FNetworkHandleBase& Handler );
FString DWORDToIpString( DWORD W );
DWORD IpStringToDWORD( const TCHAR* S );
